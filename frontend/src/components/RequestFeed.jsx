...
export default function RequestFeed({ feed = [] }) {
  return (
    <div className="card p-4 sm:p-6 flex flex-col flex-1 min-h-[320px] sm:min-h-[400px] max-h-[600px]">
      <h3 style={{ fontFamily: "'JetBrains Mono', monospace" }} className="text-sm uppercase tracking-widest text-text-muted mb-4">
        Request Log
      </h3>
      <div className="flex-1 min-h-[320px] sm:min-h-[400px] max-h-[600px] overflow-y-auto space-y-2 custom-scrollbar pr-1">
        {feed.length === 0 ? (
          ...
          feed.map((entry, i) => (
            ...
            >
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.8rem' }} className="grid grid-cols-1 sm:grid-cols-[120px_minmax(0,1fr)_180px] items-start sm:items-center gap-2 sm:gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  <span>{entry.status === 429 ? '❌' : '✅'}</span>
                  <span className="whitespace-nowrap">#{entry.requestId}</span>
                </div>

                <div className="text-text-muted min-w-0 flex flex-wrap gap-x-3 gap-y-1">
